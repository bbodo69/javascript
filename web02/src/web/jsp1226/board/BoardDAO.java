...
public class BoardDAO {
	...
	public void insertArticle(BoardDTO article) {
		// 조정, 확인이 필요한 값들 꺼내기
		int num = article.getNum();				// 글번호(새글작성 0, 댓글이면 1)
		int ref = article.getRef();				// 글 그룹1
		int re_step = article.getRe_step();		// 정렬 순서 0
		int re_level = article.getRe_level();	// 답글 레벨 0
		int number = 0;							// DB에 저장할 글 고유번호(글의 주민등록번호라고 생각하기)
		
		try {
			conn = getConnection();
			// DB에서 가장 큰 글 고유번호 가져오기(num컬럼 = 글 고유번호)
			...
			// 답글일 경우
			if(num != 0) {
				// DB에 기존에 달린 답글(re_step 0보다 큰)이 있다면, 새답글이 1이 되기 위해
			...
			// 새글일 경우
			ref = number;	// 글 그룹번호와 글 고유번호를 동일하게
			re_step = 0;
			re_level = 0;
			}
			String sql = "insert into board(num, writer, subject, email, content, pw, reg,";
	...
	public int updateArticle(BoardDTO article) {
		int x = -1;			// 1 : 수정 완료		0 : 비번오기입		-1 : 메서드 오류
		String dbpw = "";
		try {
			conn = getConnection();
			// 비번확인 (dbpw 와 넘겨받은 article 안에 있는 pw <-- 사용자가 입력한 비번 확인)
