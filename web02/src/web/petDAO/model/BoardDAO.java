...
public class BoardDAO {
	...
	public List getArticles(int startRow, int endRow) {
		...
		try {
			conn = getConnection();
			String sql = "select  num,id,content,reg,readcount,subject,r from(select num,id,content,reg,readcount,subject,rownum r from (select num,id,content,reg,readcount,subject from board order by num desc)) where r >= ? and r <= ?";
			pstmt = conn.prepareStatement(sql);
