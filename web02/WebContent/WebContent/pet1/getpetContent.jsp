...
<%@page import="web.petDAO.model.GetpetDTO"%>
<%@page import="java.util.List"%>
<%@page import="web.petDAO.model.GetpetDAO"%>
	...
	<body>
		...
		<section>
			...
			<table>
				...
				<tr>
					<td colspan="2">
					<button onclick="window.location.href='getpetModifyForm.jsp?num=<%=num%>&pageNum=<%=pageNum%>'">수정하기</button>	
					<button onclick="window.location.href='getpetDeleteForm.jsp?num=<%=num%>&pageNum=<%=pageNum%>'">삭제하기</button>	
					</td>
				</tr>
				<% 	} %>
			</table>
				...
				<%	//다음글 버튼 생성하기
					...
					if(request.getParameter("count") != null){
						...
						if(count == realCount){
							...
							if(nextNum > 0) { %>
							<button onclick="window.location.href='getpetContent.jsp?num=<%=nextNum%>&pageNum=<%=pageNum%>&count=<%=count%>'">다음글</button>	
						<% 	} %>			
						...
						<%	
							...
							if(pastNum > 0) {  %>
							<button onclick="window.location.href='getpetContent.jsp?num=<%=pastNum%>&pageNum=<%=pageNum%>&count=<%=count%>'">이전글</button>	
						<% 	}
						}else {%>
							<button onclick="window.location.href='getpetList.jsp?pageNum=<%=pageNum%>'">리스트보기</button>		 
					<% 	}
					}else{%>
