...
<body>
	...
	<table>
		<tr>
			<td><button onclick="window.location='tradeWriteForm.jsp'">글쓰기</button></td>
		</tr>
		<tr>
	...
	<table class="listTable">
		...
		<%if(session.getAttribute("sId") != null){ %>
			<td colspan="7" align="right">
				<button onclick="window.location='tradeWriteForm.jsp'">글쓰기</button>
			</td>
		<%}else{%>		
			<td colspan="7" align="right"><button onclick="window.location='tradeWriteForm.jsp'">글쓰기</button></td>
		<%} %>
		</tr>
		<tr>
			<td>게시글 번호</td>
		...
		<tr>
			...
			<td>
				...
				<%	}%>
				<a href="tradeContent.jsp?num=<%=article.getNum()%>&pageNum=<%=currentPage %>&from=tradeList"></a>
			</td>
			<td>
	...
	<%
		if(count > 0){
			...
			if(startPage > pageBlock){ %>
				<a href="tradeList.jsp?pageNum=<%= startPage-pageBlock %>" > &lt; </a>
			<%}
			for(int i = startPage; i <= endPage; i++){ %>
				<a href="tradeList.jsp?pageNum=<%=i %>" name="list"> &nbsp; <%=i %> &nbsp; </a>
			<%}
	...
	%>
		...
		<br />
		<button onclick="window.location='main.jsp'">메인 페이지로</button>
	</div>
		<br/>
		<br/>
		<br/>
