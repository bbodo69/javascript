...
<%@page import="team02.QNADTO"%>
<%@page import="team02.QNADAO"%>
...
<%-- 		<textarea cols="35" rows="1" name="reple<%=dto.getRef()%>"></textarea>	--%>			
			...
			<%if(session.getAttribute("sId").equals("1")){ %><input type="submit" value="답글"><%} %>
			<input type = "hidden" name="ref" value="<%=dto.getRef() %>">
	</form>	
	
	<form action = "qnaDelete.jsp" method="post">
	
			<%if(dto.getId().equals(session.getAttribute("sId")) || session.getAttribute("sId").equals("1")){ %>	
			...
			if(dao.getCount(dto.getRef()) != 0){
				...
				%>
					...
					<tr>
						...
						<td>								
								<%if(dto.getIdQNA().equals(session.getAttribute("sId")) || session.getAttribute("sId").equals("1")){ %>
