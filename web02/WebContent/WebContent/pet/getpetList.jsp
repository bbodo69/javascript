...
<body>
	...
	<section>
		...
		<h1>
	 <% if(sId != null){ %>
			<button onclick="window.location.href='getpetWriteForm.jsp'">작성하기</button>
	 <% } %>
		<button onclick="window.location.href='commentList.jsp'">펫시터 이용후기 보러가기</button>
		</h1>
		<table>
			<tr>    
		...
		<%
			if(count != 0){  //게시판에 글이 1개라도 존재하면 
				...
				<% 	}else{ %>				
					<img src="/web/getpetImage/empty.png" width="100"/>
				<% 	} %>
					</td>
					...
					지역 : <%=sitter.getArea() %> &nbsp; 펫시터 경력 : <%=sitter.getPetsit() %>회 &nbsp; 키워본 동물 수 : <%=sitter.getPet() %>마리 <br/>
					후기 : commentCount개 &nbsp; 평점 : <%=String.format("%.1f", sitter.getAvgpoint()) %>점
					</td>
		...
		<%
			if(count > 0){
				...
				if(startPage > pageBlock){ %>
					<a href = "getpetList.jsp?pageNum=<%=startPage-1%>"> &lt; </a>	
			<%  }
				//5. 페이지번호 반복해서 뿌려주기
				for(int i = startPage; i <= endPage; i++){ %>
					<a href = "getpetList.jsp?pageNum=<%=i %>"> &nbsp; <%= i %> &nbsp; </a>
		<% 		}
				
				//7. 뒷 페이지가 더 있으면 '>'
				if(endPage < pageCount){ %>
				<a href = "getpetList.jsp?pageNum=<%=endPage+1%>"> &gt; </a>	
		<%  	}
			}
